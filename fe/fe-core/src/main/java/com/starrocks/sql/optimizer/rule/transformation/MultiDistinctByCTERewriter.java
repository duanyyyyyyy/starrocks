...
public class MultiDistinctByCTERewriter {
    ...
    public List<OptExpression> transformImpl(OptExpression input, OptimizerContext context) {
        ...
        if (otherAggregate.size() > 0) {
            allCteConsumes.offer(
                    CTERewriterUtils.buildOtherAggregateCTEConsume(otherAggregate, aggregate,
                            cteProduce, columnRefFactory, columnRefMap));
        }
        ...
        while (allCteConsumes.size() > 1) {
            ...
            if (!hasGroupBy) {
                ...
                join = CTERewriterUtils.buildJoin(left, right, JoinOperator.INNER_JOIN);
    ...
    private OptExpression buildCountSumDistinctCTEConsume(ColumnRefOperator aggDistinctRef,
        ...
        LogicalCTEConsumeOperator cteConsume =
                CTERewriterUtils.buildCteConsume(cteProduce, cteConsumeRequiredColumns, factory);
